      ...
      balance_plan.with_lock do
        ...
        if huobi_api
          ...
          # 保证第一次 将从起始价到顶价 间隔 已经全部买入
            ...
            # 尝试将比next_should_buy_price小的正在交易的订单取消掉, 这里尝试取消未买入  间隔区间*0.6 防止在价格临界点频繁下单并取消
            balance_plan.order_plans.category_buy.where(status: [:status_created, :status_trading]).where('buy_price < ?', next_should_buy_price - balance_plan.interval_price * 0.6).each do |buying_order_plan|
