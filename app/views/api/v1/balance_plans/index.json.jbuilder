...
json.items @balance_plans do |balance_plan|
  ...
  json.full_coin_name "#{balance_plan.trade_symbol.try(:base_currency).try(:upcase)} / #{balance_plan.trade_symbol.try(:quote_currency).try(:upcase)} [count: #{balance_plan.amount}]" rescue nil
